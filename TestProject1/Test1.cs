...
namespace TestProject1
{
    ...
    public sealed class Test1
    {
        ...
        public async Task TestMethodParallel(int count, int tasks)
        {
            ...
            await File.WriteAllLinesAsync(tempFile, Enumerable.Range(0, count).Select(i => i.ToString()), Encoding.UTF8, cts.Token);
            ...
            try
            {
                ...
                var processResult = await ParallelTextFileProcessor.ProcessAsync(
                    ...
                    contextFactory: async () => await Task.FromResult(new List<int>(count / ParallelTextFileProcessor.RecommendedTasks(tasks))),
        ...
        public async Task TestMethodParallelEncoding(string encodingName, bool detectEncoding, int chunkSize)
        {
            ...
            try
            {
                ...
                await TestEncodingDetection(tempFile, encoding);
                ...
                Assert.AreEqual(testData.Length, numbers.Count);
            ...
            static async Task TestEncodingDetection(string tempFile, Encoding encoding)
            {
        ...
        public void TestMethod2(int count)
        {
            ...
            try
            {
                ...
                var sw = Stopwatch.StartNew();
                using var streamReader = new StreamReader(tempFile, Encoding.UTF8);
        ...
        public void TestEncoding()
        {
            ...
            foreach (var encodingInfo in encodings)
            {
                ...
                try
                {
                    ...
                    var (detected, _) = ParallelTextFileProcessor.DetectEncoding(bomData);
        ...
        public async Task TestMethodCountErrorInLog()
        {
            ...
            await File.WriteAllLinesAsync(targetFile, 
                Enumerable
                    .Range(0, lines)
                    .Select(i =>
                    { 
                        if (i % 1000 == 0) {
                            ++writtenErrors;
                            return $"This is line {i} with ERROR";
                        } else 
                            return $"This is line {i} without error"; 
            ...
            Assert.AreEqual(writtenErrors, errorCount);
